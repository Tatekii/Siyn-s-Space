...
📌 **哪些情况会触发回流（Reflow）？**
4. 添加/删除 DOM 元素
5. 改变 **几何属性**（如 width、height、padding、margin、border, xxSize, xxHeight）
6. 读取元素的几何信息。
	1. offsetXX
	2. clientXX
	3. scrollXX
7. 改变 **位置**（如 top、left）
8. 改变布局属性（如 position float）
9. 改变显示属性display
10. resize窗口
📌 **优化减少回流（Reflow）**
11. 使用class修改样式
...
2. `DOMContentLoaded`
	2. 当 **HTML 被完全解析** 并且 **DOM 构建完成** 后就会触发。
	3. 不会等待异步script的执行(async,defer)。
	4. 不会等待 CSS、图片、iframe 等外部资源加载完成。
```javascript
