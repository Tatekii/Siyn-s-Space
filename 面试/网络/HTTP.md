...
#### 多路复用(Multiplexing)
在单个 TCP 连接上同时并发地传输多个请求和响应。每个请求和响应都分帧传输，每个帧都有一个**流标识符 (Stream ID)**，表示该帧属于哪个请求或响应，且帧可以乱序发送和接收，最终由接收方根据帧的 Stream ID 重组成完整的消息。
😊这彻底解决了 HTTP/1.1 的队头阻塞问题(Head-of-Line Blocking, HOLB)。
